	...
	if(float(coin) > float(event_rate[0])): #
		...
		if(question_asked and user_asked):
			...
			if(int(replies_count[0]) > 0):
				...
				cursor = db.cursor()
				cursor.execute("select IFNULL(count(*),0) from user_replies where user_replies.r_idmaj != 3 and user_replies.q_id in (select uq_id from user_questions where q_id = "+str(question)+") and r_id =1")
				yes_count = cursor.fetchone()
				
				cursor.execute("select IFNULL(count(*),0) from user_replies where user_replies.r_id != 3 and user_replies.q_id in (select uq_id from user_questions where q_id = "+str(question)+") and r_idmaj =1")
				pyes_count = cursor.fetchone()
				
				
				cursor.execute("select IFNULL(count(*),0) from user_replies where user_replies.r_idmaj != 3 anduser_replies.q_id in (select uq_id from user_questions where q_id = "+str(question)+") and r_id =2")
				no_count = cursor.fetchone()
				
				cursor.execute("select IFNULL(count(*),0) from user_replies where user_replies.r_id != 3 anduser_replies.q_id in (select uq_id from user_questions where q_id = "+str(question)+") and r_idmaj =2")
				pno_count = cursor.fetchone()
				cursor.close()
				...
				logging.info(str(Unique_ID)+";predicted yes_count is "+str(pyes_count[0])+" and predicted no count is "+str(pno_count[0]))
				
				## added this for the new reputation (based on majority) // Sep 19 2017
				
			 	average_crowd_experties_voted_yes = 1 #cursor.fetchone() # is the average (yes) answer rating now 
			 	
			 	average_crowd_experties_voted_no = 1 #cursor.fetchone() # is the average (no) answer rating now

				
				logging.info(str(Unique_ID)+";Average Answer : Yes "+ str(average_crowd_experties_voted_yes)+" No "+ str(average_crowd_experties_voted_no))
				...
				cursor.execute("insert into app_ratings (question_id,app_id,yes_count,no_count,beta_yes,beta_no,timestamp) values ("+str(question)+","+str(app)+","+str(yes_count[0])+","+str(no_count[0])+","+str(average_crowd_experties_voted_yes)+","+str(average_crowd_experties_voted_no)+",UNIX_TIMESTAMP())")
				db.commit()
				cursor.close()
				##
				...
				if(int(yes_count[0]) == int(pyes_count[0]) or int(no_count[0]) == int(pno_count[0])):
				...
				elif(int(no_count[0]) > int(pno_count[0])):
					new_reputation = -1 * float(abs(int(no_count[0]) - int(pno_count[0])))/(int(no_count[0]) + int(yes_count[0]))
				elif(int(yes_count[0]) > int(pyes_count[0])):
					new_reputation = float(abs(int(yes_count[0]) - int(pyes_count[0])))/(int(no_count[0]) + int(yes_count[0]))
