	...
	if(float(coin) > float(event_rate[0])): #
		...
		if(question_asked and user_asked):
			...
			if(int(replies_count[0]) > 0):
				...
				## edited the user count based on the maximum confidence // Sep 19 2017
				cursor = db.cursor()
				cursor.execute("select IFNULL(count(*),0) from user_replies where user_replies.answer_rate >= 4.0 and user_replies.q_id in (select uq_id from user_questions where q_id = "+str(question)+") and r_id =1")
				yes_count = cursor.fetchone()
				cursor.execute("select IFNULL(count(*),0) from user_replies where user_replies.answer_rate >= 4.0 and user_replies.q_id in (select uq_id from user_questions where q_id = "+str(question)+") and r_id =2")
				...
				#cursor = db.cursor()
			 	#cursor.execute("select IFNULL(AVG(answer_rate),0) from user_replies ur join user_questions uq on ur.q_id = uq.uq_id  where uq.q_id in (select uq_id from user_questions where q_id = "+str(question)+") and ur.r_id = 1")
			 	average_crowd_experties_voted_yes = 1 #cursor.fetchone() # is the average (yes) answer rating now 
			 	#cursor.execute("select IFNULL(AVG(answer_rate),0) from user_replies ur join user_questions uq on ur.q_id = uq.uq_id  where uq.q_id in (select uq_id from user_questions where q_id = "+str(question)+") and ur.r_id = 2")
