	...
	if(float(coin) > float(event_rate[0])): #
		...
		if(question_asked and user_asked):
			...
			if(int(replies_count[0]) > 0):
				...
				## added this for the new reputation
				cursor = db.cursor()
			 	cursor.execute("select IFNULL(AVG(user_reputation),0) from Users where user_id in (select u.user_id from Users u join user_replies ur on u.user_id = ur.user_id join user_questions uq on u.user_id = uq.user_id where uq.q_id in (select uq_id from user_questions where q_id = "+str(question)+") and ur.r_id = 1)")
			 	average_crowd_experties_voted_yes = cursor.fetchone()
			 	cursor.execute("select IFNULL(AVG(user_reputation),0) from Users where user_id in (select u.user_id from Users u join user_replies ur on u.user_id = ur.user_id join user_questions uq on u.user_id = uq.user_id where uq.q_id in (select uq_id from user_questions where q_id = "+str(question)+") and ur.r_id = 2)")
			 	average_crowd_experties_voted_no = cursor.fetchone()
			 	cursor.close()
				...
				logging.info(str(Unique_ID)+";Average Crowd Experties : Yes "+ str(average_crowd_experties_voted_yes[0])+" No "+ str(average_crowd_experties_voted_no[0]))
