  ...
  exit;
      ...
      try{
        ...
        $user_intervals_up_to_date = 0;
              ...
              foreach($result as $key => $res){
                try{
                  ...
                  foreach($res as $key_2 => $res_2){
                    ...
                    $query_insert_CPC_stats = "insert into cpu_memory_stats (ua_id,vss,rss,cpu,timestamp) values ($user_app_res[0],'".$res_2['VSS']."','".$res_2['RSS']."','".$res_2['CPU']."','".$res_2['Timestamp']."')";
      	      
                    if(!mysqli_query($con,$query_insert_CPC_stats)){
                      continue;
                    }else{
                    		$query_insert_counter_log = "insert into user_logs_counter (user_app_id,feature_id) values ($user_app_res[0],2),($user_app_res[0],3),($user_app_res[0],4)";
                  ...
                  error_log($e->getMessage()." InsertCPC",0);
                  $con->rollback();
                }
              }
          ...
          if($error > 0){
            mysqli_close($con);
            error_log($error_message." InsertCPC",0);
