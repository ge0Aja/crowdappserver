  ...
  try{
    ...
    if( $user->num_rows !=null && $user->num_rows >0){
      ...
      if($_SERVER['PHP_AUTH_PW'] == 'v3Ry$t0ngP@$$w0rd!'){
        ...
        $query = "insert into alarms (user_id,app_id,ft_id,val,prt,timestamp) values ($user_res[0],$app_res[0],(select ft_id from features where feature_name = '".$thresh."'),$val_th,$prt_th,'".$time."');";
        ...
        if(mysqli_query($con,$query)){
          ...
          $mystring = system('python /usr/local/bin/process_trigger_no_proc_logging_enabled.py '.$app_id_res[0].' '.$feature_id_res[0].' '.$time.' '.$val_th.' '.$prt_th);
          exit();
        }else{
          $con->rollback();
