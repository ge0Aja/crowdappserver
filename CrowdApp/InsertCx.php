  ...
  try{
    ...
    if($user->num_rows !=null && $user->num_rows >0){
      ...
      if($_SERVER['PHP_AUTH_PW'] == 'v3Ry$t0ngP@$$w0rd!'){
          ...
          foreach($result as $key_2 => $res_2){
            ...
            $app_res = mysqli_fetch_row($app);
                ...
                foreach($res_2 as $key_3 => $res_3){
                  $query_insert_Cx_stats = "insert into connection_stats (ua_id,connection,age,timestamp) values ($user_app_res[0],'".$key_3."','".$res_3['Age']."','".$res_3['Timestamp']."')";
                ...
                $query_insert_counter_log = "insert into user_logs_counter (user_app_id,feature_id) values ($user_app_res[0],1)";
              ...
              error_log($e->getMessage()." InsertCx",0);
              $con->rollback();
            }
            //	}
        ...
        if($error > 0){
          mysqli_close($con);
          error_log($error_message." InsertCx",0);
