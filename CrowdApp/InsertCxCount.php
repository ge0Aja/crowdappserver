   ...
   if($user->num_rows !=null && $user->num_rows >0){
    ...
    if($_SERVER['PHP_AUTH_PW'] == 'v3Ry$t0ngP@$$w0rd!'){
      ...
      foreach($result as $key => $res){
        try{
          foreach ($res as $key2 => $res2){
            $con->begin_transaction(MYSQLI_TRANS_START_READ_WRITE);
            $query_package = "select app_id from Apps where package_name = '".$key2."'";
            $app = mysqli_query($con,$query_package);
            if($app->num_rows == null){
              $query_insert_package = "insert into Apps(package_name) values ('".$key2."')";
            ...
            $app_res = mysqli_fetch_row($app);
              ...
              $query_insert_cxcount = "insert into connection_count_stats (ua_id,cx_count,timestamp) values ($user_app_res[0],$res2,UNIX_TIMESTAMP())";
              if(!mysqli_query($con,$query_insert_cxcount)){
                $error += 1;
                $con->rollback();
              }else{
                $query_insert_counter_log = "insert into user_logs_counter (user_app_id,feature_id) values ($user_app_res[0],11)";
            ...
            error_log($e->getMessage()." InsertCxCount",0);
            $con->rollback();
          }
    ...
    if($error > 0){
      mysqli_close($con);
      error_log($error_message." InsertCxCount",0);
