...
if (!isset($_SERVER['PHP_AUTH_USER'])) {
  ...
  try{
      ...
      if($_SERVER['PHP_AUTH_PW'] == 'v3Ry$t0ngP@$$w0rd!'){
        ...
        foreach($result as $key => $res){
          try{
            ...
            $query_insert_UT_stats = "insert into app_usage (ua_id,interval_id,type,timestamp) values ($user_app_res[0],(select id from intervals where value =".$res['interval']."),'".$res['type']."','".$res['timestamp']."')"; //
            if(!mysqli_query($con,$query_insert_UT_stats)){
              $error += 1;
              $con->rollback();
            }else{
              $con->commit();
            ...
            error_log($e->getMessage()." InsertUT",0);
            $con->rollback();
          }
        }
      }else{
    ...
    if($error > 0){
      mysqli_close($con);
      error_log($error_message." InsertUT",0);
